...
- **Backup**: `data/` (the Matrix session and crypto store, the dynamic configuration, the ledger) and both `.env` files. Copy `billing.db` with `sqlite3 /data/billing.db ".backup /tmp/billing.db"` from inside the container, or stop the bot first; a plain copy of a live SQLite file can be inconsistent.
