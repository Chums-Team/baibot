...
- (**Feature**) [🔐 TRON wallet login](./docs/configuration/authentication.md#tron-wallet-authentication): a `user.tron` section (`private_key` or `seed_phrase`, optional `origin`) logs the bot in through the Chums homeserver's `cc.chums.login.tron` login type, for homeservers where password login is off and accounts belong to TRON wallets. The bot signs the homeserver's challenge the way TronLink's `signMessageV2` does and continues with the resulting access token; the wallet is only used when there is no saved session. The same key also yields the passphrase of the account's secret storage (the signature of `chums-ssss-v1:<user id>`, as in the Chums web client), so no `recovery_passphrase` is needed and the bot shares its encryption key backup with the web client logged in with the same wallet; a configured passphrase takes precedence. Behind the `tron-login` cargo feature, on by default.

- (**Improvement**) [Encryption keys recovered at every start](./docs/configuration/authentication.md#encryption-keys-and-recovery): with `user.encryption.recovery_passphrase` set, the bot imports its cross-signing keys and the room key backup key from the account's secret storage each time it starts, creating the secret storage when the account has none, instead of only on a fresh login. A bot whose `data/` was lost reads the history of its encrypted rooms again. The log reports the outcome (`Recovery: …`); with `recovery_reset_allowed` the bot also repairs a secret storage or key backup it cannot use, otherwise it stops with a hint.
